    ...
    if (dataJson["status"] == 1){
        ...
        if (hospitals.length > 0) {
            ...
            for (i=0; i < hospitals.length; i++) {
                optionLine = "<option data-hospital-address='"+hospitals[i]["hospital-address"]+"' value=" + hospitals[i]["hospital-id"] + ">" + hospitals[i]["hospital-name"] + "</option>";
	    ...
	    $("#hospital-id").val($("#hospital-name-select option:selected").val());
	    $("#doctors-address").val($("#hospital-name-select option:selected").attr("data-hospital-address"));
	}
        else {
